...
$$
The real value $\lambda$ is a **hyper parameter**.  When $\lambda=0$ we have the usual OLS (**ordinary least sum of squares**).  Otherwise the sum of squares of the parameters influences the result.
...
$$
It would also be pretty natural to produce a short hand for our probabilities too:

$$
p_i = \textrm{P}(D=i)
$$
(Ask me if that last equation doesn't make immediate sense to you)

Notice that the six events we have described have two important properties:

1. Together they *cover* the entire sample space
1. The events have no outcomes in common
...
1. Roll the die
1. Update our best guess for every $p_i$
1. Repeat... lots
...
f(x) = g(x) + h(x)
$$
Then the function $f(x)$ is built out of $g(x)$ and $h(x)$ combined with addition.  It turns out that the **derivative** of $f(x)$ is built out of the derivatives of $g(x)$ and $h(x)$ in the same fashion
...
h=0.75
delta=0.1
my.rect<-rectGrob(x=0.1,y=0.1,width=w,height=h,just = j,vp=vp,gp=gpar(fill="transparent"))
my.rect2<-rectGrob(x=0.1,y=0.1+h-delta,width=w-delta,height=delta,just=j,vp=vp,gp = gpar(lty = "dashed",fill="gray90",alpha=0.4))
my.rect3<-rectGrob(x=0.1+w-delta,y=0.1,width=delta,height=h-delta,just=j,vp=vp,gp = gpar(lty = "dashed",fill="gray90",alpha=0.4))

my.rect4<-rectGrob(x=0.1+w-delta,y=0.1+h-delta,width=delta,height=delta,just=j,vp=vp,gp = gpar(lty = "dashed",fill="gray70",alpha=0.4))
...
The width of the entire section is $f(x+\Delta x)$.  The first section is $f(x)$.  So the width of the darkest region is an approximation to $f(x+\Delta x)-f(x)\approx f'(x) \Delta x$
#4 6 18 19 23
...
$$
Now the most interesting one is what happens when we compose $f(g(x))-f(g(x+\Delta x))$.  Try to think about what that would mean.
...
\end{aligned}
$$
The rate of change here is $ac$.  Notice this is the rate of change of $f(x)$ times the rate of change of $g(x)$... Even though for these simle functions the rate of change is the same for every value of $x$, if we are very clever we might notice that we care about the rate of change of $f$ and $g(x)$.  So, it's more accurate to write:  $f'(g(x))g'(x)$.
...
```{r, echo=FALSE}
sliderInput("x", "Value of x", min = 0, max = 1.2, value = 0,step=0.05,animate=list(interval=200,loop=TRUE))
plotOutput("animPlot",height="600px",width="600px")
```
...
Further notice that the $x$ and $y$ limits on the graph are not identical... so the exact value of the slope is a bit hard to eyeball-- you really do need to think about the equation $$\textrm{slope}=\frac{\textrm{rise}}{\textrm{run}}$$
in order to calculate the exact value.  Regardless, positive slopes and negative slopes are unaffectd by this x-scale vs y-scale issue.
